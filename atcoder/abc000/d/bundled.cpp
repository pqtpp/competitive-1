...
void solve() {
    int n, m; cin >> n >> m;
    vi a(n); cin >> a;
    vvi dp(n+1, vi(m+1, inf)), dp2(n+1, vi(m+1, inf));
    // dp[i][j]: i番目まで決定した時、総和がjになる最小手数 ただしi番目は消してない
    // dp2[i][j]: i番目まで決定した時、総和がjになる最小手数 ただしi番目は消してる
    dp[0][0] = 0;
    rep1(i, n) rep(j, m+1) {
        // 消す
        dp2[i][j] = min(dp[i-1][j]+1, dp2[i-1][j]);
        // 消さない
        if (a[i-1] <= j) {
            dp[i][j] = min(dp[i-1][j-a[i-1]], dp2[i-1][j-a[i-1]]);
        }
    }
    rep1(i, m) {
        int ans = min(dp[n][i], dp2[n][i]);
        if (ans == inf) ans = -1;
        cout << ans << nl;
    }
}
